...
def build_row(row_number, number_of_columns, use_cjk=False, use_emoji=False):
    row = {}
    for column_number in range(0, number_of_columns):
        row.update(
            {
                build_column_name(column_number, use_cjk=use_cjk, use_emoji=use_emoji): chaos_monkey(build_value(row_number, column_number))
            }
        )
    return row
...
def build_value(row_number, column_number):
    cell_type = get_type(column_number)
    if cell_type == str:
